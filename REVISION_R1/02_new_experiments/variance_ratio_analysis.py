...
#   khong thien vi : trung vi 0.97, khoang 10-90% = [0.65, 1.34]
#   thien vi vua   : trung vi 0.59, khoang 10-90% = [0.38, 0.85]
HI, LO = 0.85, 0.65
...
def main():
    ...
    print("=" * 72)
    print(f"  {len(reps)} lan lap x {K} fold = {len(d)} quan sat")
    print("=" * 72)
    print(f"  chenh lech trung binh (mlp - arch): {d.mean():+.2f} cycles")
    print(f"  so fold thuan chieu               : {int((d>0).sum())}/{len(d)}")
    print(f"\n  var giua cac fold                 : {var_fold:8.2f}")
    print(f"  var giua cac lan lap (quan sat)   : {var_rep:8.2f}")
    print(f"  var giua cac lan lap (ky vong)    : {exp_rep:8.2f}")
    print(f"  TY LE                             : {ratio:8.3f}")

    print(f"\n  Nguong tu mo phong (n_rep=20):")
    print(f"    ty le > {HI}  -> fold gan DOC LAP  -> t-test thuong hop le")
    print(f"    ratio < {LO}  -> a persistent bias is present, so Nadeau-Bengio applies")

    if ratio > HI:
        verdict = "FOLD GAN DOC LAP -> dung t-test ghep cap thuong"
    elif ratio < LO:
    ...
    print(f"  t-test ghep cap thuong : {d.mean():+.2f} "
  ...
  da phan doi viec tuyen bo vuot troi tu khoang tin cay chong nhau.
""")
